...
class FileSystem implements MasterStatic {
     ...
     * @return void
     *
     * @note On Windows, if $new_name already exists, it must be writable, otherwise rename() fails and issues E_WARNING.
     */
     ...
     *
     * @caution pathInfo() is locale aware, so for it to parse a path containing multibyte characters correctly, the
     * matching locale must be set using the setlocale() function.
     *
     * @note pathInfo() operates naively on the input string, and is not aware of the actual filesystem, or path
     * components such as "..".
     ...
     *
     * Attempts to change the group of the file or folder $path to $group.
     * Only the superuser may change the group of a file arbitrarily; other users may change the group of a file to
     ...
     * permissions, owner, group, or other metadata from the inode is updated. See also lastModified()
     * (which is what you want to use when you want to create "Last Modified" footers on web pages) and lastAccessed().
     ...
     * @note Note that time resolution may differ from one file system to another.
     */
    final public static function setLastAccessedAndModification (string $path, int $last_accessed = null, int $last_modified = null):bool {
     ...
     * Only the superuser may change the group of a symlink arbitrarily. Other users may change the group of a
     * symlink to any group of which that user is a member.
     * @since 1.0.0
     *
     * @param non-empty-string $path <p>
     * Path to the symlink.
    ...
    final public static function statistics (string $path, bool $symlink = false):array {
        ...
        return Arr::filter(
            ...
            fn ($value, $key) => DataIs::string($key),
            true
        );
     ...
     *
     * When you use stat(), lstat(), or any of the other functions listed in the affected functions list (below), PHP
     * caches the information those functions return in order to provide faster performance. However, in certain cases,
     * you may want to clear the cached information. For instance, if the same file is being checked multiple times
     * within a single script, and that file is in danger of being removed or changed during that script's operation,
     * you may elect to clear the status cache. In these cases, you can use the clearCache() function to clear the
